import { distributeProportionally, percentOfPaise } from './money';
import {
  BookingBreakup,
  BookingRequest,
  LineBreakup,
...
function findSlabRate(referencePricePaise: number, slabs: TaxSlab[]): number {
...
function validate(request: BookingRequest, tiersByName: Map<string, SeatTier>): PricingError[] {
 ...
 */
export function priceBooking(show: Show, request: BookingRequest, config: PricingConfig): PricingResult {
  ...
  const lines = request.lines.map((l) => {
    const tier = tiersByName.get(l.tierName) as SeatTier;
    return {
      tierName: tier.name,
      quantity: l.quantity,
  ...
  const festivalRequested =
    request.applyFestivalDiscount && config.offers.festivalDiscount.enabled
  ...
  if (request.isMember) {
    ...
    const rawPercentOff = percentOfPaise(remainingAfterFestival, percent);
    memberDiscount = Math.min(rawPercentOff, capPaise, remainingAfterFestival);
  }

  const totalDiscount = festivalDiscount + memberDiscount;

  const perLineDiscount = distributeProportionally(
    totalDiscount,
    lines.map((l) => l.grossAmountPaise)
  );

  // --- Tax + fee per line --------------------------------------------
  const lineBreakups: LineBreakup[] = lines.map((l, idx) => {
    const discountApplied = perLineDiscount[idx];
    const taxableAmount = l.grossAmountPaise - discountApplied;

    const referencePrice =
      config.taxSlabBasis === 'discounted' ? Math.round(taxableAmount / l.quantity) : l.unitPricePaise;
    ...
    const ticketGst = percentOfPaise(taxBase, gstPercent);
    const feeGst =
      config.fee.taxation === 'separate' ? percentOfPaise(convenienceFee, config.fee.separateGstPercent ?? 0) : 0;
  ...
  const summary = lineBreakups.reduce(
    (acc, l) => {
...
export function commitBooking(show: Show, request: BookingRequest): void {
  for (const line of request.lines) {
    const tier = show.tiers.find((t) => t.name === line.tierName);
    if (!tier) throw new Error(`Unknown tier ${line.tierName}`);
    if (line.quantity > tier.availableSeats) throw new Error(`Tier ${line.tierName} sold out`);
